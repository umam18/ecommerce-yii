...
class SiteController extends BaseController {
    ...
    public function actionRegister() {
        if (UsniAdaptor::app()->user->isGuest) {
            ...
            if ($formDTO->getIsTransactionSuccess() == true) {
                FlashUtil::setMessage('success', UsniAdaptor::t('userflash', 'Anda berhasil mendaftarkan akun baru . Silakan masuk dengan Akun Anda tersebut.'));
    ...
    public function actionMeasurementProfile() {
        $searchModel = new MeasurementProfileSearch();
        $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
        return $this->render('/measurement-profile/index', [
                    'searchModel' => $searchModel,
                    'dataProvider' => $dataProvider,
        ]);
    }

    public function actionCreateMeasurementProfile() {
        ...
        if ($model->load(UsniAdaptor::app()->request->post())) {
            ...
            if ($model->save(false)) {
                $searchModel = new MeasurementProfileSearch();
                $dataProvider = $searchModel->search(Yii::$app->request->queryParams);
                return $this->redirect('measurement-profile');
            }
        } else {
            return $this->render('/measurement-profile/_form', ['model' => $model,]);
        }
    }
